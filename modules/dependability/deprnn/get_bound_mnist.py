...
class RNN(nn.Module):
    ...
    def get_preactivation_output(self, X):
        ...
        with torch.no_grad():
            ...
            for i in range(seq_len):
                pre_h[:, i + 1, :] = (torch.matmul(self.W_ax, X[:, i, :].unsqueeze(2)).squeeze(2) + b_ax +
    ...
    def attachData(self, X):
        ...
        if torch.numel(X) == (X.shape[0] * self.input_dimension):
            ...
            raise Exception(
                'The input dimension must be %d' % self.input_dimension)
    ...
    def compute2sideBound(self, eps, p, v, X=None, Eps_idx=None):
        ...
        with torch.no_grad():
            ...
            if type(eps) == torch.Tensor:
                ...
                yU = yU + idx_eps[v - 1] * eps.unsqueeze(1).expand(-1, s) * torch.norm(W_ax, p=q, dim=2)  # eps ||A^ {<v>} W_ax||q
                yL = yL - idx_eps[v - 1] * eps.unsqueeze(1).expand(-1, s) * torch.norm(W_ax, p=q, dim=2)  # eps ||Ou^ {<v>} W_ax||q
            else:
            ...
            if not (v == 1):
                ...
                for k in range(v - 1, 0, -1):
                    if k == v - 1:
                        ...
                        kl, bl, ku, bu = get_bound.getConvenientGeneralActivationBound(self.l[k], self.u[k], self.activation)
                    ...
                    if type(eps) == torch.Tensor:
                        # eps is a tensor of size N
                        yU = yU + idx_eps[k - 1] * eps.unsqueeze(1).expand(-1, s) * torch.norm(
                            torch.matmul(A, W_ax), p=q,
                            dim=2)  # eps ||A^ {<k>} W_ax||q
                        yL = yL - idx_eps[k - 1] * eps.unsqueeze(1).expand(-1, s) * torch.norm(
                            torch.matmul(Ou, W_ax), p=q,
    ...
    def computeLast2sideBound(self, eps, p, v, X=None, Eps_idx=None):
        with torch.no_grad():
            ...
            for k in range(v - 1, 0, -1):
                ...
                if type(eps) == torch.Tensor:
                    # eps is a tensor of size N
                    yU = yU + idx_eps[k - 1] * eps.unsqueeze(1).expand(-1, t) * torch.norm(
                        torch.matmul(A, W_ax), p=q,
                        dim=2)  # eps ||A^ {<k>} W_ax||q
                    yL = yL - idx_eps[k - 1] * eps.unsqueeze(1).expand(-1, t) * torch.norm(
                        torch.matmul(Ou, W_ax), p=q,
                        dim=2)  # eps ||Ou^ {<k>} W_ax||q
                else:
                    yU = yU + idx_eps[k - 1] * eps * torch.norm(
                        torch.matmul(A, W_ax), p=q, dim=2)  # eps ||A^ {<k>} W_ax||q
                    yL = yL - idx_eps[k - 1] * eps * torch.norm(
                        torch.matmul(Ou, W_ax), p=q, dim=2)  # eps ||Ou^ {<k>} W_ax||q
                ## second term
                yU = yU + torch.matmul(A, torch.matmul(W_ax, X[:, k - 1, :].view(N, n, 1))).squeeze(2)  # A^ {<k>} W_ax x^{<k>}
                yL = yL + torch.matmul(Ou, torch.matmul(W_ax, X[:, k - 1, :].view(N, n, 1))).squeeze(2)  # Ou^ {<k>} W_ax x^{<k>}
                ## third term
    ...
    def getMaximumEps(self, p, true_label, target_label, eps0=1, max_iter=100,
        ...
        with torch.no_grad():
            ...
            if gx0_trick == True:
                ...
                self.b_f = Parameter(
                    self.b_f[true_label] - self.b_f[target_label])
                print("after gx0_trick W_fa size = {}".format(self.W_fa.shape))
                print("after gx0_trick b_f size = {}".format(self.b_f.shape))
            yL, yU = self.getLastLayerBound(u_eps, p, X=X, clearIntermediateVariables=True, Eps_idx=Eps_idx)
            if gx0_trick:
                lower = yL[idx, idx]
            ...
            while (increase_u_eps.sum() > 0):
                ...
                yL, yU = self.getLastLayerBound(u_eps[increase_u_eps], p, X=X[increase_u_eps, :],
                ...
                if gx0_trick:
                    ...
                    true_lower = yL[
                        torch.arange(num), true_label[increase_u_eps]]
                    target_upper = yU[
                        torch.arange(num), target_label[increase_u_eps]]
            ...
            while (search.sum() > 0):
                ...
                yL, yU = self.getLastLayerBound(eps, p, X=X[search, :], clearIntermediateVariables=True, Eps_idx=Eps_idx)
