...
class PrivateRNN(RecurrentNeuralNetworks):
    ...
    def train(self, train_data, test_data):
        ...
        self.model = SimpleDeepESNClassifier(
            num_classes=4, input_scaling=1.8,
            leaky=0.4, spectral_radius=0.9,
            units=1000, layers=3)
        ...
        eps, delta = self._compute_privacy_budget(
            self.epochs * X_train.shape[0] // self.batch_size,
            X_train.shape[0])
        print(f'n_samples={X_train.shape[0]}, delta={delta:.6f}. Computed epsilon={eps:.2f}')
